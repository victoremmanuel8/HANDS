function startRegistration() {
    ...
    function nextStep(step) {
        if (step === 1) {
            ...
            checkStep3();
                    ...
                    var nomeValido = (nomeRegex.test(nome) && !palavrasProibidas.some(palavra => nome.toLowerCase().includes(palavra.toLowerCase())) || palavrasPermitidas.includes(nome.toLowerCase())) && nome.length >= 2;

                    // Verifica se o sobrenome contém palavras proibidas, exceto as excepcionais
                    var sobrenomeValido = nomeRegex.test(sobrenome) && (!palavrasProibidas.some(palavra => sobrenome.toLowerCase().includes(palavra.toLowerCase())) || palavrasPermitidas.includes(sobrenome.toLowerCase())) && sobrenome.length >= 2;

                    // Verifica se os campos não estão vazios e seguem as regras de validação
                    if (nome !== '' && sobrenome !== '' && email !== '' && nomeValido && sobrenomeValido && emailRegex.test(email)) {
                ...
                function exibirTextoErro() {
                    escreverTexto("Verifique se todos os campos foram digitados corretamente", document.getElementById('texto5'), .1, () => {
                ...
                function exibirTextoPalavra() {
                    escreverTexto("Nome não permitido, verifique se foi digitado corretamente ou se existe campos vazios", document.getElementById('texto6'), .1, () => {
                        document.querySelector('.balao1').style.display = 'block';
    ...
    document.getElementById('nome').addEventListener('keypress', function(event) {
        ...
        if ((keyCode >= 48 && keyCode <= 57) || /[!@#$%^&*()[\]\-+=_'":;\/\\{}<>|,.¨?`ªº§¹²³£¢¬₢]/.test(keyChar)) {
            event.preventDefault();
        }
    });

    document.getElementById('sobrenome').addEventListener('keypress', function(event) {
        // Obtém o código da tecla digitada
        var keyCode = event.keyCode || event.which;
        var keyChar = String.fromCharCode(keyCode);
    ...
    if (senha !== '' && confirmarSenha !== '' && senha === confirmarSenha && senhaRegex.test(senha)) {
        document.getElementById('step2').classList.add('hidden');
        document.getElementById('step3').classList.remove('hidden');
    } else {
        if (!senhaRegex.test(senha)) {
            balaoErro.textContent = 'A senha deve ter pelo menos 6 caracteres e conter pelo menos 1 letra maiúscula. Por favor, verifique e tente novamente. 😔';
        } else {
